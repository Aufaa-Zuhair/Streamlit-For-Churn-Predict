import churn_streamlit as st
import joblib
import pandas as pd

st.title('Bank Service Churn Predict')
st.write('This app will predict if the customer will churn or not')

# === Load Model ===  -> menggunakan model sesuai dengan 
...
# === Layout: 2 Columns === -> untuk membuat jumlah layout yang digunakan
col1, col2 = st.columns(2, gap="large")

# --- Customer Profile ---  -> berisikan biography customer
with col1:
...
# === Data Preparation ===  -> untuk menyiapkan data yang akan diprediksi
input_data = {
...
# === Convert binary columns to Yes/No for display ===  -> untuk memperlihatkan hasil yes/no pada saat review data muncul
...
# === Custom Formatters ===  -> untuk mengcustom format data yang di khususkan untuk kolom numerik
formatters = {
...
# === Prediction Button (Styled with CSS) ===  -> membuat tombol predict
st.markdown(
    """
    <style>
